...
function checkTeacherConflict(targetTeacherName, targetCellId, currentClass) {
    ...
    const targetDay = targetCellId.split('-')[1];

    for (let className in allScheduleData) {
        if (className === currentClass) continue;
        const classSchedule = allScheduleData[className] || {};
        
        // Aynı saat çakışması kontrolü
        if (classSchedule[targetCellId] && classSchedule[targetCellId].teacherName === targetTeacherName) {
            return className;
        }
        
        // Yeni kural: Bir öğretmen aynı gün içinde FARKLI BİR SINIFIN dersine GİREMEZ.
        for (let cell in classSchedule) {
            if (cell.endsWith('-' + targetDay) && classSchedule[cell].teacherName === targetTeacherName) {
                return className + ' (Öğretmen bu gün sadece bu sınıfa girebilir)';
            }
        }
    }
    return null;
...
function autoGenerateSchedule() {
    if(confirm(`${activeClassName} sınıfı için rastgele program yerleştirilecek. Mevcut program silinsin mi?`)) {
        ...
        let grade = 9;
        if (activeClassName.startsWith('10')) grade = 10;
        else if (activeClassName.startsWith('11')) grade = 11;
        else if (activeClassName.startsWith('12')) grade = 12;

        const shuffledDays = [...DAYS].sort(() => 0.5 - Math.random());
        let dayConstraints = {};
        
        if (grade === 12) {
            let activeDays = shuffledDays.slice(0, 2);
            DAYS.forEach(d => {
                if (activeDays.includes(d)) {
                    dayConstraints[d] = { morning: 'Mixed', afternoon: 'Mixed' };
                } else {
                    dayConstraints[d] = { morning: 'Empty', afternoon: 'Empty' };
                }
            });
        } else if (grade === 11) {
            let meslekDays = shuffledDays.slice(0, 3);
            DAYS.forEach(d => {
                if (meslekDays.includes(d)) {
                    dayConstraints[d] = { morning: 'Meslek', afternoon: 'Meslek' };
                } else {
                    dayConstraints[d] = { morning: 'Ortak', afternoon: 'Ortak' };
                }
            });
        } else {
            // 9 ve 10. sınıflar için 1.5 gün Meslek
            let fullMeslekDay = shuffledDays[0];
            let halfMeslekDay = shuffledDays[1];
            let isMorning = Math.random() > 0.5;
            
            DAYS.forEach(d => {
                if (d === fullMeslekDay) {
                    dayConstraints[d] = { morning: 'Meslek', afternoon: 'Meslek' };
                } else if (d === halfMeslekDay) {
                    dayConstraints[d] = { morning: isMorning ? 'Meslek' : 'Ortak', afternoon: isMorning ? 'Ortak' : 'Meslek' };
                } else {
                    dayConstraints[d] = { morning: 'Ortak', afternoon: 'Ortak' };
                }
            });
        ...
        // Sadece Kültür (Ortak) derslerinde ARA SIRA erken çıkmaları için
        // Haftada en fazla 1 Ortak günün öğleden sonrasından 2-3 saat keselim
        let ortakDays = Object.keys(dayConstraints).filter(d => dayConstraints[d].afternoon === 'Ortak');
        if (ortakDays.length > 0) {
            let earlyLeaveDay = ortakDays[Math.floor(Math.random() * ortakDays.length)];
            // 2 veya 3 saat erken
            dayConstraints[earlyLeaveDay].earlyLeave = Math.random() > 0.5 ? 2 : 3;
        }

        for (let day of DAYS) {
            let constraint = dayConstraints[day];
            const halves = [
                { type: constraint.morning, startHour: 1, isAfternoon: false },
                { type: constraint.afternoon, startHour: 6, isAfternoon: true }
            ];

            let usedLessonsToday = new Set();
            
            for (let half of halves) {
                if (half.type === 'Empty') continue;

                let targetSum = 5;
                if (half.isAfternoon && half.type === 'Ortak' && constraint.earlyLeave) {
                    targetSum = 5 - constraint.earlyLeave;
                }
                ...
                if (half.type === 'Meslek') {
                    // Meslek derslerinde asla erken çıkış yok, saatler tam verilmeli (büyük bloklar)
                    let pattern = Math.random();
                    if (targetSum === 5) {
                        if (pattern < 0.3) blocks = [5];
                        else if (pattern < 0.6) blocks = [3, 2];
                        else blocks = [2, 3];
                    } else {
                        blocks = [targetSum];
                    }
                } else {
                    // Ortak veya Karışık ise 1 veya 2'li bloklar
                    while(sum < targetSum) {
                        let size = Math.random() > 0.4 ? 2 : 1;
                        if (sum + size > targetSum) size = targetSum - sum;
                        blocks.push(size);
                        sum += size;
                    }
                }

                let currentHour = half.startHour;
                
                for (let blockSize of blocks) {
                    let shuffledPool = [...lockedPool].sort(() => 0.5 - Math.random());
                    
                    if (half.type === 'Meslek') {
                        shuffledPool = shuffledPool.filter(p => p.branch === branchKey);
                    } else if (half.type === 'Ortak') {
                        shuffledPool = shuffledPool.filter(p => p.branch === 'Ortak');
                    }
                    // Mixed => Filtre yok
                    
                    let placed = false;
                    for (let pick of shuffledPool) {
                        if (usedLessonsToday.has(pick.lessonName)) continue;
                        
                        let conflictFree = true;
                        for (let h = 0; h < blockSize; h++) {
                            let cellId = `${currentHour + h}-${day}`;
                        ...
                        if (conflictFree) {
                            ...
                            usedLessonsToday.add(pick.lessonName);
                            placed = true;
                            break;
                        }
                    }
                    ...
                    currentHour += blockSize;
                }
            }
        }
        
        // BOŞLUKLARI KÜLTÜR (ORTAK) DERSLERİYLE DOLDURMA (Gaps Fix)
        for (let day of DAYS) {
            let constraint = dayConstraints[day];
            if (constraint.morning === 'Empty') continue; 
            
            let maxHour = 10;
            if (constraint.earlyLeave) maxHour = 10 - constraint.earlyLeave;
            
            let ortakPool = lockedPool.filter(p => p.branch === 'Ortak');

            for (let hour = 1; hour <= maxHour; hour++) {
                let cellId = `${hour}-${day}`;
                
                if (!scheduleData[cellId]) {
                    let shuffledPool = [...ortakPool].sort(() => 0.5 - Math.random());
                    
                    let placed = false;
                    for (let pick of shuffledPool) {
                        let isUsed = false;
                        for (let h = 1; h <= 10; h++) {
                             if (scheduleData[`${h}-${day}`] && scheduleData[`${h}-${day}`].lessonName === pick.lessonName) {
                                 isUsed = true; break;
                             }
                        }
                        if (!isUsed && !checkTeacherConflict(pick.teacherName, cellId, activeClassName)) {
                             scheduleData[cellId] = pick;
                             placed = true;
                             break;
                        }
